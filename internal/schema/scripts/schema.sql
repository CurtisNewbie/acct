...
CREATE TABLE `cashflow_statistics` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'primary key',
  `user_no` varchar(32) NOT NULL DEFAULT '' COMMENT 'user no',
  `agg_type` varchar(32) NOT NULL DEFAULT '' COMMENT 'aggregation type: MONTHLY, YEARLY, WEEKLY',
  `agg_range` varchar(32) NOT NULL DEFAULT '' COMMENT 'aggregation range, year or month value',
  ...
  `deleted` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'record deleted',
  PRIMARY KEY (`id`),
  KEY `user_agg_type_range_idx` (`user_no`, `agg_type`, `agg_range`)
